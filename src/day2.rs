...
lazy_static! {
    static ref COMMAND_RE: Regex = Regex::new(r"^(?P<direction>forward|up|down) (?P<value>\d{1,2})$").unwrap();
}
...
pub fn day2part1() {
    ...
    commands.iter().for_each(|cmd| {
        match cmd {
            SubCommand::Forward(value) => { x += *value as i32; }
            SubCommand::Down(value) => { y += *value as i32 }
            SubCommand::Up(value) => { y -= *value as i32 }
        }
    });

    println!("Day1 Part2 Solution: position={}, depth{}, answer={}", x, y, x*y);
}
...
pub fn day2part2() {
    ...
    commands.iter().for_each(|cmd| {
        match cmd {
            SubCommand::Forward(value) => { x += *value as i32; y += aim * *value as i32 }
            SubCommand::Down(value) => { aim += *value as i32 }
            SubCommand::Up(value) => {  aim -= *value as i32 }
        }
    });

    println!("Day1 Part2 Solution: position={}, depth={}, aim={}, answer={}", x, y, aim, x*y);
